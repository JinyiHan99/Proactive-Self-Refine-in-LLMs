...
client = OpenAI(base_url="http://0.0.0.0:59824/v1/", api_key="dummy")
prompt_template = '''
        ### Question:
        {Question}
...
def llm_eval(sys_prompt, example):
    if "I believe the above solution is correct" in example['second_answers_without_oracle']:
        user_prompt = prompt_template.format(Question = example['question'], Ground_Truth = example['std'], Answer = example['first_answer'])
    else:
        user_prompt = prompt_template.format(Question = example['question'], Ground_Truth = example['std'], Answer = example['second_answers_without_oracle'])
...
def main(args):
    ...
    for data_name in args.data_names:
        ...
        data_path = f"{args.main_data_path}/{data_name}_without_oracle.json"
        print(data_path)
        ...
        for batch_start in tqdm(range(0,len(dataset),32)):
            ...
            for example in temp_data:
                ...
                cur_eval_data.append({"question":example['question'], "std":example['std'], "first_answer":example['first_answer'], "second_answer_without_oracle":example['second_answers_without_oracle'], "score":score})
            #判断模型第一次答案的得分情况
