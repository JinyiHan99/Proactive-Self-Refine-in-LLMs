...
def main(args):
    ...
    for data_name in args.data_names:
        ...
        data_path = ""
         # 加载数据集
        dataset = []           
        dataset_path = "/mnt/data/kw/wxy/self_refine/datasets"
        if data_name == "mmlu":
            data_path = f"{dataset_path}/mmlu/select_mmlu_test.json"
            problem_key = "Question"
            std_key = "Ground_truth"
            first_prompt = gen_prompt_config['first_gen_mmlu']

        elif data_name == "humaneval":
            data_path = f"{dataset_path}/humaneval/HumanEval.json"
            problem_key = "prompt"
            std_key = "canonical_solution"
            first_prompt = gen_prompt_config['first_gen_humaneval']

        elif data_name == "drop":
            data_path = f"{dataset_path}/drop/test_sampled_313.jsonl"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_drop']

        elif data_name == "xsum":
            data_path = f"{dataset_path}/xsum/test_select313.jsonl"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_xsum']

        elif data_name == "gsm8k":
            data_path = f"{dataset_path}/gsm8k/test.jsonl"  # 替换为实际路径
            problem_key="question"
            std_key="answer"
            first_prompt = gen_prompt_config['first_gen_math']

        elif data_name == "math":
            data_path = f"{dataset_path}/math/math.json"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_math']
    
        elif data_name == "aime24":
            data_path = f"{dataset_path}/aime24/aime24.jsonl" 
            problem_key = "problem"
            std_key = "solution"
            first_prompt = gen_prompt_config['first_gen_math']

        elif data_name == "arc-challenge":
            data_path = f"{dataset_path}/arc/ARC-c/two_keys_test.jsonl"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_arc']

        elif data_name == "gpqa":
            data_path = f"{dataset_path}/gpqa/prompt_messages.jsonl"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_gpqa']

        elif data_name == "wino":
            data_path = f"{dataset_path}/winogrande/dev_two_keys.jsonl"
            problem_key = "question"
            std_key = "answer"
            first_prompt = gen_prompt_config['first_gen_wino']

        elif data_name == "commonsenseqa":
            data_path = f"{dataset_path}/commonsenseqa/two_keys_version.jsonl"
            problem_key = "question"
            std_key = "answer"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="/mnt/data/kw/lty/ckp/qwen_sft/checkpoint-129",
    ...
    parser.add_argument("--output_path", type=str, default="/mnt/data/kw/wxy/self_refine/Qwen2.5-14B/PTR"),
